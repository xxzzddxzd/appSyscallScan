...
def init_session():
    try:
        ...
        if platform == 'ios' or platform == 'android':
            try:
                print('222')
                device = frida.get_usb_device(-1) # added timeout to wait for 3 seconds
            except Exception as e:
...
try:
    ...
    if session:
        ...
        if script:
            ...
            if spawn == 1 and pid:
                device.resume(pid)
            app.run() #Start WebServerhost='0.0.0.0',port= 5000,debug=False
except Exception as e:
    print((colored('[ERROR] ' + str(e), 'red')))
    traceback.print_exc()
    sys.exit(1)
